    ...
    public sealed class LabLandscapeApplier : MonoBehaviour
    {
        ...
        void BuildWorld(GizaComplex.Spawn which)
        {
            ...
            Debug.Log(
                ...
                " West/East/Central/Menkaure Field mastabas; Hemiunu G4000; Senedjemib G2370; Ankhhaf G7510 East Field; Debehen Central Field; Menkaure quarry/ramp schematic; Gisr el-Mudir; Khufu Trial Passages; Heit el-Ghurab workers village + bakeries/workshops + Wall of the Crow; Osiris Shaft (schematic); Petrie relieving graffiti + Dream Stele attested text; West Field + boat-pit survey heatmaps; GizaSurveyAnomalies speculative thermal solids OFF by default; SpeculativeUnderworld OFF by default." +
        ...
        static void AddTeleports(GameObject root)
        {
            ...
            Transform gisr = root.transform.Find("GisrElMudir");
            if (gisr == null)
            {
                GameObject ge = GizaComplex.FindNamed("GisrElMudir");
                gisr = ge != null ? ge.transform : null;
            }
            if (gisr != null)
            {
                for (int i = 0; i < gisr.childCount; i++)
                {
                    Transform gch = gisr.GetChild(i);
                    if (gch == null)
                        continue;
                    string gn = gch.name.ToLowerInvariant();
                    if (gn.Contains("sand") || gn.Contains("apron") || gn.Contains("gate"))
                        AddTeleport(gch.gameObject);
                }
            }
